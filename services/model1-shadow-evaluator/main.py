...
def start_shadow():
    ...
    model_path = "/models/anomaly_detection/model1_20f_healthy_candidate/best_vae_model1_20f_healthy.pth"
    ...
    conf = {
        ...
        'group.id': 'shadow-evaluator-group',
        'auto.offset.reset': 'latest'
    }
    consumer = Consumer(conf)
    consumer.subscribe(['enriched.features'])
        ...
        while True:
            ...
            if len(buffer[key]) >= seq_len:
                ...
                out_rec = {
                    ...
                    "model_checkpoint": "best_vae_model1_20f_healthy.pth",
                    "scaler_version": "model1_20f_scaler.pkl",
                    "sequence_timestamps": [w["window_end"] for w in seq],
                    "max_adjacent_gap": max((ts_list[i] - ts_list[i-1]).total_seconds() for i in range(1, len(ts_list))) if len(ts_list) > 1 else 0.0
                }
                
                out_dir = "/artifacts/feature_validation/model1_fault_evaluation_v3"
                os.makedirs(out_dir, exist_ok=True)
                with open(os.path.join(out_dir, "evaluation_windows.jsonl"), "a") as f:
                    f.write(json.dumps(out_rec) + "\n")
                    
                captured += 1
                logger.info(f"Captured {captured} baseline window.")
                if captured >= 1:
                    logger.info("Successfully evaluated baseline. Terminating safely.")
                    return
